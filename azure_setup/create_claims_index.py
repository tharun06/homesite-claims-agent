...
def create_index():
    ...
    delete_url = f"{AZURE_SEARCH_ENDPOINT}/indexes/{CLAIMS_INDEX}?api-version=2024-07-01"
    headers = {
    ...
    url = f"{AZURE_SEARCH_ENDPOINT}/indexes/{CLAIMS_INDEX}?api-version=2024-07-01"

    index = {
        "name": CLAIMS_INDEX,
        "fields": [
            {"name": "id", "type": "Edm.String", "key": True, "filterable": True, "retrievable": True},
            {"name": "claim_number", "type": "Edm.String", "filterable": True, "retrievable": True},
            {"name": "content", "type": "Edm.String", "searchable": True, "retrievable": True},
            {"name": "content_vector", "type": "Collection(Edm.Single)", "searchable": True,
            "retrievable": True, "dimensions": 1536, "vectorSearchProfile": "hnsw-profile"},
            {"name": "status", "type": "Edm.String", "filterable": True, "retrievable": True},
            {"name": "estimated_amount", "type": "Edm.Double", "filterable": True, "retrievable": True},
        ],
        "vectorSearch": {
            "algorithms": [
                {
    ...
    if response.status_code in (200, 201, 204):
        print(f"✅ Index '{CLAIMS_INDEX}' created successfully")
    else:
